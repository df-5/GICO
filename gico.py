        ...
        with open("%sdatabase/update" %(_PATH), "r") as update:
            for line in update.readlines():
                ...
                if int(bytes_size) == online_size:
                    ...
                    menu = myshell.Menu(0)
                    menu.add_entry(" \'%s\' needs update %d/%d" % (file_name, int(bytes_size), int(online_size) ), lambda:None)
                    menu.show()
                    value = menu.run("Continue")
                    if value:
...
class Console(object):
    ...
    def do_get(self, args):
        ...
        try:    
            ...
            print_error("Query: invaild syntax, RTFM")
            return 
...
def print_error(msg):
    print("\033[01;37;41m[X] %s\033[00m" %(msg) )

def print_info(msg):
    print("\033[01;37m[I] %s\033[00m" % (msg) )
